...
The code uses stage names consistently. **Parsing** turns source into syntax and may retain recovered syntax alongside syntax errors. **Lowering** converts an external AST shape into evaluator-owned forms. **Evaluation** performs bounded abstract Python execution. A **Python Environment** is the filesystem context selected by an executable, virtual-environment root, or system prefix; `python/environment.rs` inspects that context without invoking Python. **Extraction** derives Django-specific facts from syntax or evaluated Python facts. **Discovery** assembles project-level facts from configured and observed sources. **Resolution** connects names and references to modules, Templates, definitions, or Models; an internal **search** is the ordered traversal used to reach that result. Compilation is reserved for producing executable or lower-level programs and does not describe the Python evaluator.
...
Push diagnostics use a coalescing queue with monotonic tickets independent of client document versions. Mutations schedule work under the session lock; computation uses fresh snapshots, and a single publisher rejects superseded tickets before delivery. Pull-client fallback tickets follow intervening edits until delivery without permanently enabling push diagnostics. Document readers share immutable `Arc<TextDocument>` buffers; edits mutate in place unless a retained reader requires copy-on-write.

A single coalescing reload worker runs full Project reloads and intrinsic-only re-primes without overlap; queued full reloads dominate re-primes. Readiness guarantees registration inventory and structural grammar, not every validation detail. Synchronized Python content edits under registered project/search roots request re-priming and diagnostic republishing even outside published eager coverage, so lazily discovered helpers cannot escape invalidation. Settings and membership changes still require full discovery. Before coverage is known, every synchronized Python edit is treated conservatively as Project work. Edited helpers enter temporary coverage for failure/retry revision tracking; successful publication replaces it with current eager coverage. Only successful priming for the current generation publishes readiness; stale or cancelled work cannot release requests, and failures remain explicit failed states.

Optional cache warm-up and coalesced diagnostic-refresh transport run outside the reload worker. Database snapshots never cross client progress or refresh waits; optional warm-up reacquires current-generation readiness after progress setup.
...
The Project Facts layer. This crate owns mechanical facts about a Django project: the `Project` Salsa input, Python Environment and search-path discovery, module resolution, Django settings extraction, template directories, Template Libraries, discovered template files, template origins/resolution, template tag/filter extraction, model graph extraction, and Django Discovery phases with their domain progress metadata. Its internals are organized by Django domain: `settings/` handles settings-source extraction and source resolution, `templates/` handles template origins, directories, Template Libraries, registrations, tag rules, filter arities, and tag/filter definitions, and `models/` handles Django model graph extraction. The Model Graph remains available through its Salsa query, but routine Django Discovery and IDE Cache Warm-up do not scan model sources or build it; a feature that consumes Model facts triggers that work.
...
Descriptors retain declaration identity separately from the callable presented to registration. Unknown inner decorators prevent signature and compile-body inference; outer decorators run after registration. The signature query models the resolved canonical `django.template.defaultfilters.stringfilter` API as preserving the signature Django inspects through `inspect.unwrap`, in both installed packages and source checkouts. Resolution checks establish the decorator's identity; they do not verify its implementation. This supplies signature evidence only, not compile-body equivalence or navigation. Incompatible changes to the canonical implementation and runtime monkeypatching are outside this model.
...
Settings extraction jointly demands `INSTALLED_APPS` and `TEMPLATES`. A conservative backward slice selects whole top-level regions, retaining the entire prefix before an unclassified effect and executing selected statements in source order. Within retained prefixes, globally unobserved assignments can avoid constructing fresh collections only when their leaves cannot carry aliases; their forward effect pass still runs. Imported modules keep full evaluation, parsing remains whole-file, and syntax recovery and root-reaching import cycles retain the full evaluator's behavior.
...
These full products are demand-driven. A separate structural query fuses builtins, Block Specs, and configured fallback for eager topology. It retains full fusion for the exceptional case where an unknown extracted closer with intermediates gains a configured closer and Tag Rule presence affects the merge. Occurrence and completion detail still use the original builtin → rules → blocks → configured order and full-spec agreement across feasible backends.

For a project-backed Template, semantic analysis builds one tracked `TemplateAnalysisProjection`. A fixed-point loop resolves Tag occurrences in the source, discovers effective loader occurrences by `TagRole`, and converges the ordered loaded-library state with a sparse occurrence grammar. Tag fact collection resolves each name once per semantic `(visible load prefix, symbol name)` and reuses that contextual result across repeated occurrences, while retaining occurrence-specific structure, arguments, spans, and source identity. The explicit projectless structure seam instead builds its sparse grammar and `TemplateTree` directly in one pass; it does not construct project-correlated Tag or Filter facts. The converged project-backed product correlates:

- the `TemplateTree` and captured closing occurrences;
- ordered Loaded Libraries;
- sparse Tag facts, including effective specs and availability.

The tracked `scoped_filter_facts` query derives Filter availability and arity from that converged tree and load state only when validation needs them, with the same per-load-prefix/name deduplication. Filter-name completion can resolve inventory and availability without requesting arity.
...
`TemplateValidator` consumes the converged projection and demands Filter facts as needed. It checks load scoping, arguments, Filter Arity, `{% if %}` expressions, and `{% extends %}` positioning without rerunning structural/load convergence. Opaque contents never enter the active occurrence stream. Structural diagnostics are emitted only from the converged pass, so fixed-point retries cannot duplicate them.
...
Project configuration, Python Environment and search-path discovery, module resolution, and source-derived Django facts live in `crates/djls-project/`. `Project` is a Salsa input holding the project root, Python Environment selection, Django settings module, resolver-owned `SearchPaths`, and manual tag-spec configuration. `djls-project` statically maps an executable, virtual-environment root, or system prefix to ordered `site-packages`/`dist-packages` roots; it reads `pyvenv.cfg` when system packages are enabled but never invokes Python. `djls-project` also owns the `TemplateLibraryCatalog` derived from Django settings, source files, and installed packages. `ScopedTemplateLibraries` is a borrowed view over that shared catalog plus compact feasible-backend selections for one Template; deriving a scoped view does not clone selected libraries or definitions. Files outside configured Template roots intentionally use project-inventory scope. Imperative Django Discovery functions synchronize external project state into Salsa inputs; tracked Project and Semantic queries derive equality-bearing products from those inputs.

### `crates/djls-ide`

IDE features: completions, diagnostics, folding ranges, snippets, goto definition, find references, and cache warm-up for responsive startup. This crate owns two synchronous production preparation seams. Intrinsic priming walks active Template Library identities, evaluates registration inventory and structural vocabulary, and returns eager source coverage used by server readiness; it performs no per-Template parsing, projection, or validation. Full Tag Rules and Filter Arity remain lazy except where structural fusion requires the compatibility fallback. Tag-name completion narrows candidate names and supported closer spellings by prefix before requesting full specs; an empty prefix may intentionally demand all matching snippets. One-shot project Template analysis composes intrinsic priming with shared Template indexing and reports whether a Project was available; callers need no preparation internals. The crate also translates internal domain knowledge into LSP-shaped output that editors can consume.
...
1. The `Project` input stores the project root, Python Environment selection, Django settings module, search paths, and manual tag-spec configuration.
...
4. The registration scanner produces indexed definition facts for each Template Library identity; independent queries derive structural and validation detail when requested. Shared catalog assembly stores definition-name references and backend correlation, not copied semantic specs. Model Graph extraction is separate from this discovery sequence and runs only when a feature requests Model facts.
...
- **Tag Rules** — argument count constraints (min/max positional args), required keywords, choice-constrained positions, and `as var` support
- **Block Specs** — intermediate and end tags, and whether a body is opaque
...
**Architecture Invariant:** Static analysis does not execute project code to derive facts. Corpus setup uses Python to install dependencies, then the analyzer resolves their source statically. Focused tests can supply controlled source through `ProjectFixture` for edge cases. JSON fixtures remain appropriate for isolated extraction projections.
...
The corpus is downloaded rather than checked into the repository. `just corpus sync` prepares pinned source and each project's dependency environment, reusing ready entries on subsequent runs. `--refresh` rebuilds environments and re-resolves unlocked dependencies.
...
Current coverage follows the production contract across crates. It distinguishes eager inventory/structure from first-use validation detail, repeated cached requests, and the structural compatibility fallback; priming performs no per-Template work. CLI-style parallel validation starts after structural priming, and LSP requests wait for current-generation readiness. Template-pipeline tests cover same-revision memoization, whitespace backdating, meaningful load/Block Tag/Filter edits, unrelated Template and Python edits, sparse projection locality, and unchanged scoped-library backdating. Server tests cover Python helpers outside eager coverage, stale and cancelled generations, failure/retry admission, race-free readiness watches, and full-reload dominance over queued re-primes.
