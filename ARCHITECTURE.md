...
`djls-project` depends on `djls-source` for filesystem/source access, but it does not depend on `djls-semantic`. That one-way seam lets semantic analysis consume observed source facts without project discovery needing to know about template validation, scoping, or diagnostics. Each Template Library has an owned `(Option<File>, PythonModuleName)` identity and equality-bearing definition, Tag, and Filter facts. Private lifetime-bound interned keys memoize per-library queries without leaking Salsa lifetimes into domain values. `TemplateLibraryCatalog` assembles backend-correlated catalog evidence and shared definition-name indexes; it does not hold one project-global semantic Tag or Filter result.
