...
Descriptors retain declaration identity separately from the callable presented to registration. Unknown inner decorators prevent signature and compile-body inference; outer decorators do not change a callable already registered. Lazy signature evidence recognizes Django's exactly resolved canonical `stringfilter` export, including aliases, independently of its search-root kind. This models Django's unwrapped-signature API contract; it does not verify the wrapper implementation, establish body equivalence, or provide a navigation target. A sole canonical definition whose body violates that API contract is outside this model, whether installed or in a source checkout. Registration-derived `simple_block_tag` closers remain available independently of the user callable. Parse recovery is a separate uncertainty and does not by itself discard useful declaration facts. Wrapper dependencies participate in detail invalidation without opening inventory or adding eager body analysis. Implicit registration-name inference remains a separate heuristic.
