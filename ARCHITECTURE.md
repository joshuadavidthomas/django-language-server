...
Descriptors retain declaration identity separately from the callable presented to registration. Unknown inner decorators prevent signature and compile-body inference; outer decorators run after registration. The signature query recognizes Django's canonical `stringfilter` export and aliases in installed packages and source checkouts. It assumes the wrapper preserves `__wrapped__`; it does not analyze the wrapper body or enable compile-body inference or navigation. A modified canonical definition that breaks this contract is not detected.
