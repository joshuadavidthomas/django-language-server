...
### Single config loader and tolerant caller policy
- Bookmark: `startup-rethink` has advanced past the original structured-load slice; current cleanup removes the temporary tolerant loader shape.
- Scope: keep one `djls_conf::Settings::load` config-loading API returning `Result<Settings, Vec<SettingsLoadError>>`. Deleted `LoadedSettings` / `load_settings_for_root`; moved tolerant fallback policy to `djls-project` discovery and documented non-fatal recovery policy for `djls-server` configuration reload.
- Validation:
...
- Follow-ups/blockers: Phase 3C2 should lower `SettingsLoadError` into project-owned discovery facts without reverse-engineering strings. LSP startup still initializes DB-wide settings from client options. Discovery loads per-root config through `Settings::load` and falls back to client/default settings on error. `didChangeConfiguration` should keep last known-good DB settings on config load failure so the server can recover when the user fixes the file.
...
- Validation:
  ...
  - `cargo test -p djls-db enrichment_provider` passed.
  - `cargo test -p djls-server python_source_models_startup_progress_reports_lifecycle_over_loading_events` passed.
...
- Review/reference follow-up:
  - Ousterhout review found one must-fix: `compute_tag_specs` read tag-spec config through an untracked settings mutex after old Project removal. Fixed with a Salsa-visible tag-spec config field on the stable `Project` input, updated when tag-spec settings change.
...
- Implement Phase 3C as four hard gates:
  - **3C1 single config loader**: `djls-conf` owns config file/schema loading through `Settings::load`; tolerant fallback lives in callers.
...
- In `djls-conf`, keep one config loading API: `Settings::load(root, overrides) -> Result<Settings, Vec<SettingsLoadError>>`. Do not add a second tolerant loader or outcome type. Strict vs tolerant behavior is caller policy, not a config-loading concern.
- Model config load failures as `SettingsLoadError` variants for I/O, parse, schema/deserialization, and unsupported config shapes. Preserve source path when known so tolerant callers can lower failures into their own issue types without reverse-engineering strings.
...
- For each root from `ProjectDiscoveryLoadRequest`, call `djls_conf::Settings::load(&root, Some(client_settings.clone()))` outside the session lock. On a per-root config error, record a typed `ConfigLoadFailed` issue with a project-owned `ProjectConfigLoadError` converted from `djls_conf::SettingsLoadError`, then continue discovery with the client/default settings for that root. The fallback is owned by `djls-project` discovery policy, not `djls-conf`.
...
- Extend the `didChangeConfiguration` restart path so changed client/default settings restart root-scoped discovery through the same loading graph. The server must not die or reset to broken defaults when config loading fails; it logs/reports the `SettingsLoadError`, keeps the last known-good DB settings, and allows the next configuration change to recover. The new run must reload structured root settings, apply new `ProjectDiscoverySetData` only on coherent success, and reject superseded discovery applies from older runs.
...
**Tests and stop gate**:
- **3C1 gate**: stop after `djls-conf` single-loader tests prove `Settings::load` loads project/user config, applies overrides, preserves failure source path, and returns typed `SettingsLoadError` categories without any `LoadedSettings`/tolerant-loader API: `cargo test -p djls-conf settings`.
...
**Phase 3C1 gate**
- [x] `djls-conf` settings tests preserve loaded settings, failure source path, and typed error category through `Settings::load` only. Discovery and server tests own tolerant fallback behavior.
...
#### Automated Verification
- [x] Enrichment merge tests pass: `cargo test -p djls-project enrichment`
- [x] Inspector provider/translation tests pass in the infrastructure owner and prove only `ProjectEnrichmentDraft`/typed issues cross the provider-to-domain seam: `cargo test -p djls-db enrichment_provider` or the equivalent provider-module test.
