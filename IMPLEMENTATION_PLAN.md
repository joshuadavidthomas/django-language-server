...
- [x] **M19.16** Final validation: `cargo build -q`, `cargo clippy -q --all-targets --all-features -- -D warnings`, `cargo test -q` — all green (745 passed, 0 failed, 7 ignored). Verified no `HelperCache` references in production code (`grep` returns empty).

## M20 — Rename djls-extraction → djls-python

**RFC:** `.agents/rfcs/2026-02-09-rfc-extraction-salsa-integration.md`
**Plan file:** `.agents/plans/2026-02-09-m20-rename-crate.md`

Mechanical rename — no logic changes. The crate sits alongside `djls-templates` as a SourceDb-level crate analyzing Python sources.
