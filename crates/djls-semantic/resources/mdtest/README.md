...
Snapshots run against the curated validation fixture in `src/testing.rs`, not a live inspected Django project. That keeps tests deterministic. Scenarios that assert "requires load" mean "requires load in this fixture, where the library is not configured as a template builtin." The `unreadable-library` suite runs against a fixture that adds one open library, kept apart because an open library changes what every other scenario reports. Long term, we may add fixtures that exercise more real project discovery and settings behavior.
