...
pub(crate) fn discover_module_search_paths(
    ...
    for explicit_python_path in explicit_python_paths {
        let path = normalize_module_search_path(project_root, explicit_python_path);
        if path.is_dir() {
            push_module_search_path(
                &mut search_paths,
...
pub(crate) fn resolve_module(
    ...
    let resolved = resolve_module_fact(&requested, search_paths, project_root);
    ModuleResolution {
        requested,
        resolved,
    }
...
#[must_use]
fn normalize_module_search_path(project_root: &Utf8Path, path: &Utf8Path) -> Utf8PathBuf {
    if path.is_absolute() {
        path.to_path_buf()
    } else {
