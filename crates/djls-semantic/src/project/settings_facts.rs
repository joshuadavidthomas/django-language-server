...
",
        ...
        write_file(
            &root.join("djls.toml"),
            r#"django_settings_file_patterns = ["project/settings/dev.py"]"#,
        );
        ...
        let Fact::Known {
            ...
            panic!(
                "expected discovered settings module, got {:?}",
                environments[0].django_settings_module
            );
        };

        let facts =
            extract_settings_facts_for_module(settings_file, settings_module, &root, &search_paths);
