...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) enum InspectorFailureKind {
...
pub fn load_runtime_project_enrichment(db: &dyn Db, project: Project) -> ProjectEnrichment {
    let Some(request) = runtime_enrichment_request(db, project) else {
        return ProjectEnrichment::RuntimeUnavailable;
    };
    ...
    match result {
        ...
        Err(kind) => {
            ...
            ProjectEnrichment::InspectorFailed
        }
    }
}

#[tracing::instrument(level = "info", skip_all, fields(outcome))]
fn runtime_enrichment_request(db: &dyn Db, project: Project) -> Option<RuntimeEnrichmentRequest> {
...
impl InspectorZipapp {
    ...
    fn create() -> Result<Self, InspectorFailureKind> {
        let mut file = NamedTempFile::with_prefix("djls_inspector_")
            .map_err(|_| InspectorFailureKind::SubprocessFailed { status: None })?;
...
impl InspectorCommand {
    fn for_request(request: &RuntimeEnrichmentRequest) -> Result<Self, InspectorFailureKind> {
    ...
    fn run(self) -> Result<InspectorEnrichment, InspectorFailureKind> {
        self.spawn()?.run()
    }

    fn spawn(mut self) -> Result<InspectorProcess, InspectorFailureKind> {
        let child = self
            .command
            .spawn()
            .map_err(|_| InspectorFailureKind::SubprocessFailed { status: None })?;
...
impl InspectorProcess {
    ...
    fn run(&mut self) -> Result<InspectorEnrichment, InspectorFailureKind> {
        self.write_queries()?;
        ...
        match enrichment.recv_timeout(Self::TIMEOUT) {
            ...
            Err(mpsc::RecvTimeoutError::Timeout) => {
                ...
                Err(InspectorFailureKind::TimedOut)
            }
            Err(mpsc::RecvTimeoutError::Disconnected) => {
                Err(InspectorFailureKind::SubprocessFailed { status: None })
            }
        }
    }

    fn write_queries(&mut self) -> Result<(), InspectorFailureKind> {
        let result = (|| {
            let stdin = self.child.stdin.as_mut()?;
            writeln!(stdin, "{}", json!({ "query": "template_libraries" })).ok()?;
            Some(())
        })();
        result.ok_or(InspectorFailureKind::SubprocessFailed { status: None })?;
    ...
    fn read_enrichment_async(
        ...
        mpsc::Receiver<Result<InspectorEnrichment, InspectorFailureKind>>,
        InspectorFailureKind,
    > {
        let stdout = self
            .child
            .stdout
            .take()
            .ok_or(InspectorFailureKind::SubprocessFailed { status: None })?;
        let span = tracing::Span::current();
    ...
    fn wait(&mut self) -> Result<(), InspectorFailureKind> {
        let Some(status) = self
            ...
            self.waited = true;
            return Err(InspectorFailureKind::TimedOut);
        };
        self.waited = true;
        ...
        if status.success() {
            ...
            Err(InspectorFailureKind::SubprocessFailed {
                status: status.code(),
            })
        }
    }
...
impl<T: for<'de> Deserialize<'de>> InspectorResponse<T> {
    fn parse_line(line: Option<std::io::Result<String>>) -> Result<T, InspectorFailureKind> {
        let Some(line) = line else {
            return Err(InspectorFailureKind::InvalidJson);
        };
        let Ok(line) = line else {
            return Err(InspectorFailureKind::SubprocessFailed { status: None });
        };
        ...
        response.data.ok_or(InspectorFailureKind::InvalidJson)
    }
}
