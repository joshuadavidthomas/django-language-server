...
pub(crate) fn find_executable(name: &str) -> Result<Utf8PathBuf, WhichError> {
    #[cfg(not(test))]
    {
...
#[cfg(test)]
pub(crate) fn env_var(key: &str) -> Result<String, VarError> {
    #[cfg(not(test))]
    {
        std::env::var(key)
    }
...
pub(crate) mod mock {
    ...
    pub(crate) fn find_executable_mocked(name: &str) -> Result<Utf8PathBuf, WhichError> {
    ...
    pub(crate) fn env_var_mocked(key: &str) -> Result<String, VarError> {
...
mod tests {
    ...
    fn test_env_mock_default_error_if_unmocked() {
        let _guard = MockGuard;
        let result = env_var("UNMOCKED_VAR");
        assert!(matches!(result, Err(VarError::NotPresent)));
    }
