...
pub enum InvalidName {
    ...
    #[error("template name must be relative, got absolute: {0}")]
    TemplateNameIsAbsolute(String),
    #[error("template name cannot contain parent directory components")]
    TemplateNameContainsParentComponent,
}
...
impl TemplateName {
    pub fn parse(name: &str) -> Result<Self, InvalidName> {
        ...
        if path.is_absolute() {
            return Err(InvalidName::TemplateNameIsAbsolute(trimmed.to_string()));
        }
        if path
            .components()
            .any(|component| component.as_str() == "..")
        {
            return Err(InvalidName::TemplateNameContainsParentComponent);
        }
        Ok(Self(trimmed.to_string()))
    }
...
mod tests {
    ...
    #[test]
    fn template_name_accepts_path_like_names_but_rejects_unsafe_paths() {
        assert_eq!(
            TemplateName::parse("templates/admin/index page.html")
                .unwrap()
                .as_str(),
            "templates/admin/index page.html"
        );
        assert_eq!(
            TemplateName::parse("/absolute.html"),
            Err(InvalidName::TemplateNameIsAbsolute(
                "/absolute.html".to_string()
            ))
        );
        assert_eq!(
            TemplateName::parse("../secret.html"),
            Err(InvalidName::TemplateNameContainsParentComponent)
        );
    }
