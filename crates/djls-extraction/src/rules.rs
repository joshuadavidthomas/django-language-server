    ...
    if patterns::is_len_call(right, split_var).is_some() {
        if let Some(n) = patterns::extract_int_literal(left) {
            return Some(match op {
                ...
                CmpOp::Lt => RuleCondition::MinArgCount {
                    min: (n + 1) as usize,
                },
                CmpOp::LtE => RuleCondition::MinArgCount {
                    min: n as usize,
                },
                CmpOp::Gt => RuleCondition::MaxArgCount {
                    max: (n - 1) as usize,
                },
                CmpOp::GtE => RuleCondition::MaxArgCount {
                    max: n as usize,
                },
                _ => return None,
            });
...
fn negate_condition(cond: RuleCondition) -> RuleCondition {
    match cond {
        ...
        RuleCondition::LiteralAt { index, value, negated } => RuleCondition::LiteralAt {
            index,
            value,
            negated: !negated,
        },
...
"#;
        ...
        assert!(
            ...
            "Expected LiteralAt condition with index 1, value 'as', negated=true"
        );
    }
}