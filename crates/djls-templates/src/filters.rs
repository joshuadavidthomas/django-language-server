...
use crate::quotes::QuoteEscapeMode;
...
pub(crate) fn split_variable_expression(content: &str) -> impl Iterator<Item = (&str, u32)> {
    split_on_unquoted_delimiter_with_offsets(content, '|', QuoteEscapeMode::LiteralBackslash)
        .into_iter()
...
pub(crate) fn parse_filter(raw: &str, base_offset: u32) -> Result<Filter, FilterParseError> {
    ...
    let colon_pos = first_unquoted_delimiter_index(trimmed, ':', QuoteEscapeMode::LiteralBackslash);
