...
fn extract_option_checks(
    ...
    check_test(&if_stmt.test);

    for clause in &if_stmt.elif_else_clauses {
        if let Some(test) = &clause.test {
            check_test(test);
        } else if crate::analysis::rules::body_raises_exception(&clause.body) {
