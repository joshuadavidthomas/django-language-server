    ...
    async fn formatting(
        ...
        let edits = self
            .with_session(|session| {
                ...
                let result = match settings.format().backend() {
                    djls_conf::FormatBackend::Djangofmt => {
                        djls_format::format_template(source.as_str(), file.path(db))
                    }
                };

                match result {
                    Ok(FormatOutcome::Changed(formatted)) => vec![ls_types::TextEdit::new(
                        full_document_range(
                            source.as_str(),
                            file.line_index(db),
