...
- **Internal**: Moved `check_file` orchestration and diagnostic rendering from `djls-ide` / CLI into `djls-db`, matching the ty-style architecture where the db crate owns the shared check pipeline. Simplified `collect_diagnostics` signature (dropped `nodelist` parameter). Removed `djls-ide`, `djls-templates`, and `djls-project` dependencies from the CLI binary.
