...
- Added `env_file` configuration option for loading environment variables from a `.env` file into the inspector process. Solves failures when Django settings read from `os.environ` (e.g., `DJANGO_SECRET_KEY`) and the editor doesn't inherit the user's shell environment.
