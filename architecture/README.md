...
Options:
- `--include-tests` - Include test crates (`*-test`, `*-tests`) in the graph

The graph shows:
- Local workspace crates grouped into namespace clusters
- Crates colored by namespace (border) and tag (fill)
- External dependencies from `stow.toml` `graph.external_deps` placed inside each namespace that uses them
