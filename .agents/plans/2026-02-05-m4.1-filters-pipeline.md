...
flowchart TB
    subgraph M2Pattern["INSPECTOR DATA FLOW - M2 PATTERN"]
        ...
        subgraph Refresh["refresh_inspector - SIDE-EFFECT, NOT TRACKED"]
            Step1["1. Query Python: inspector::query db, TemplateInventoryRequest<br/>SINGLE round trip - returns tags + filters + libraries + builtins"]
            Step2["2. Compare with current Project.inspector_inventory"]
            Step3["3. If changed: project.set_inspector_inventory db .to new_inventory<br/>Single atomic setter call"]
            Step1 --> Step2 --> Step3
        end
        
        subgraph Project["Project - SALSA INPUT"]
            Inventory["inspector_inventory: Option InspectorInventory<br/>- tags: Vec TemplateTag<br/>- filters: Vec TemplateFilter<br/>- libraries: HashMap String, String<br/>- builtins: Vec String"]
        end
        
        subgraph Tracked["Tracked Queries - READ PROJECT ONLY"]
            Compute["compute_tag_specs db, project<br/>→ reads project.inspector_inventory db"]
        end
